...
def _chunk_scan_chunk_state_bwd_dx_kernel(
    ...
    offs_dstate = tl.arange(
        0,
        (BLOCK_SIZE_DSTATE if IS_TRITON_22 and BLOCK_SIZE_DSTATE <= 128 else BLOCK_SIZE_K),
    )
    ...
    if IS_TRITON_22 and BLOCK_SIZE_DSTATE <= 128:
        ...
        for k in range(0, dstate, BLOCK_SIZE_K):
            b = tl.load(
                b_ptrs,
                mask=(offs_m[:, None] < chunk_size_limit) & (offs_dstate[None, :] < dstate - k),
                other=0.0,
            )
            dstates = tl.load(
                dstates_ptrs,
...
def _mamba_chunk_scan_lr_combined_fwd(
    x,
    dt,
    A,
    B,
